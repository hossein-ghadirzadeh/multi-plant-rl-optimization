...
#             next_left_state, next_right_state, reward_left, reward_right, done, info = env.step(action_left, action_right)  # PongEnv
...
def train():
    ...
    env = PongEnv(render_mode=True, max_points=3)
    state_size = 5
    action_size = 3
    ...
    # Track rally lengths and reaction times
    rally_lengths_left = []
    rally_lengths_right = []
    reaction_times_left = []
    ...
    for ep in range(1, MAX_EPISODES + 1):
        ...
        while not done:
            ...
            # Update rally lengths & reaction times if available
        ...
        # ---------------- Per-episode output (LastR + Score) ----------------
        ...
        # ---------------- Every 50 episodes: output averages ----------------
        if ep % 50 == 0:
            avg_left_reward = np.mean(episode_rewards_left[-50:])
            avg_right_reward = np.mean(episode_rewards_right[-50:])
            avg_rally_left = np.mean(rally_lengths_left[-50:])
            avg_rally_right = np.mean(rally_lengths_right[-50:])
            avg_react_left = np.mean(reaction_times_left[-50:])
            avg_react_right = np.mean(reaction_times_right[-50:])

            print(f"--- Episode {ep} Averages (last 50) ---")
        ...
        # ---------------- Evaluation ----------------
        if ep % EVAL_EVERY_EPISODES == 0:
            win_rate_right, metrics = evaluate(dqn_agent, q_agent, render=False, episodes=10)
            print(f"--- Eval after ep {ep}: Right(DQN) win rate {win_rate_right:.3f} | "
...
def evaluate(dqn_agent, q_agent, render=True, episodes=10):
...
def watch_agent_play(dqn_agent, q_agent, games=10):
    ...
    for g in range(games):
        ...
        while not done:
            ...
            action_right = dqn_agent.act(state_right, epsilon=0.0)
            state_left, state_right, _, _, done, info = env.step(action_left, action_right)
        print(f" Match {g+1}: Winner = {info.get('winner')}")
        time.sleep(1.0)
    env.close()
